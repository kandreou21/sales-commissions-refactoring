...
public class RepresentativeWindow extends JDialog {
	...
	private AppLauncherWindow inputDialog;
	private Representative selectedRepresentative;
	@SuppressWarnings("unused")
	private String fileType;
	
	public RepresentativeWindow(AppLauncherWindow dialog, Representative representative, String fileTypeFlag) {
		inputDialog = dialog;
	...
	protected void okButtonPressed(ActionEvent evt) {
		totalSales = totalSalesCheckBox.isSelected() ? selectedRepresentative.calculateTotalSales() : -1;
	    totalItems = totalItemsCheckBox.isSelected() ? selectedRepresentative.calculateTotalItems() : -1;
	    shirtSales = shirtRadio.isSelected() ? selectedRepresentative.calculateShirtsSales() : -1;
	    skirtSales = skirtRadio.isSelected() ? selectedRepresentative.calculateSkirtsSales() : -1;
	    coatsSales = coatRadio.isSelected() ? selectedRepresentative.calculateCoatsSales() : -1;
	    trousersSales = trousersRadio.isSelected() ? selectedRepresentative.calculateTrouserSales() : -1;
	    commission = commissionCheckBox.isSelected() ? selectedRepresentative.calculateCommission() : -1;
		
		ReportWindow rs = new ReportWindow(this, selectedRepresentative, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
	...
	private void appendFile() {
		...
		receipt.setReceiptID(Integer.valueOf(receiptIDTextField.getText()));
		receipt.setDate(dateTextField.getText());
		receipt.setKind(kindTextField.getText());
		receipt.setSales(Integer.valueOf(salesTextField.getText()));
		receipt.setItems(Integer.valueOf(itemsTextField.getText()));
		...
		address.setStreetNumber(Integer.valueOf(numberTextField.getText()));
		
		selectedRepresentative.getReceiptFileUpdater().setReceipt(receipt);
		selectedRepresentative.getReceiptFileUpdater().appendFile();
	}
	...
	private void addReceipt(){
		...
		try{
			...
			selectedRepresentative.getReceipts().add(receipt);
			numOfReceipts++;
