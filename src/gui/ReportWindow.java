...
public class ReportWindow extends JDialog {
	...
	private RepresentativeWindow representativeWindow;
	...
	public ReportWindow(final RepresentativeWindow sw, Representative representative,double tSales,int tItems,
			float shirtS,float skirtS,float trousersS,float coatsS,double com) {
		representativeWindow = sw;
	...
	private void outputTXTButtonPressed(ActionEvent evt) {
        JFileChooser fileChooser = createFileChooser("Save TXT File", "txt");
        ...
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            ...
            saveTXTFile(fileToSave.getAbsolutePath());
        }
    }

    private void outputXMLButtonPressed(ActionEvent evt) {
        JFileChooser fileChooser = createFileChooser("Save XML File", "xml");
        int userSelection = fileChooser.showSaveDialog(this);
        if (userSelection == JFileChooser.APPROVE_OPTION) {
            File fileToSave = fileChooser.getSelectedFile();
            ...
            saveXMLFile(fileToSave.getAbsolutePath());
        }
    }

    private JFileChooser createFileChooser(String title, String extension) {
        JFileChooser fileChooser = new JFileChooser();
        fileChooser.setDialogTitle(title);
        fileChooser.setFileFilter(new FileNameExtensionFilter(title, extension));
        return fileChooser;
    }

    private void saveTXTFile(String filePath) {
        TXTReportWriter makeTXTFile = new TXTReportWriter(selectedRepresentative);
        makeTXTFile.setFullPathName(filePath);
    ...
    private void saveXMLFile(String filePath) {
        XMLReportWriter makeXMLFile = new XMLReportWriter(selectedRepresentative);
        makeXMLFile.setFullPathName(filePath);
	...
	private void cancelButtonPressed(ActionEvent evt) {
		representativeWindow.setVisible(true);
		this.dispose();	
	}	
}
